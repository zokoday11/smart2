...
const COPY = {
  brand: "Assistant IA",
  nav: { features: "Fonctionnalités", process: "Comment ça marche", pricing: "Tarifs", login: "Connexion", tryFree: "Essayer gratuitement" },
  hero: {
    badge: "Ultra rapide • Moderne • Efficace",
    titleLine1: "Génère tes CV, lettres",
    titleGradient: "et candidatures en 1 clic",
    subtitle: "Une interface premium, un assistant intelligent, et un workflow complet pour postuler plus vite — sans prise de tête.",
    ctaPrimary: "Créer un compte",
    ctaSecondary: "Accéder à l’application",
  },
  mock: {
    dashboard: "Dashboard",
    cvs: "CV",
    ...
    proMode: "MODE PRO",
    hello: "Bonjour 👋",
    subtitle: "Tout est prêt pour booster tes candidatures",
    assistantActive: "Assistant actif",
    readyToApply: "Prêt à postuler",
    placeholder: "Décris ton objectif…",
    match: "MATCH",
    sent: "Envoyé",
    generatingPitch: "Génération en cours…",
  },
  features: {
    title: "Tout ce qu’il te faut pour postuler",
    subtitle: "CV, lettres, suivi, et automatisation — dans une seule plateforme.",
    f1: { title: "Génération instantanée", desc: "Crée CV & lettres propres, modernes et cohérents en quelques secondes." },
    f2: { title: "Mode vocal", desc: "Dicte ton besoin, l’IA structure et rédige." },
    f3: { title: "Statistiques", desc: "Suis ce qui marche : taux de réponses, versions, envois." },
    f4: { title: "Sécurité", desc: "Connexion sécurisée, contrôle des accès, et règles anti-abus." },
  },
  process: {
    title: "Comment ça marche",
    s1: { title: "1) Décris ton objectif", desc: "Le poste visé, ton profil, ton style — en 30 secondes." },
    s2: { title: "2) Génère et ajuste", desc: "CV, lettre, email — tu modifies si besoin, en direct." },
    s3: { title: "3) Postule plus vite", desc: "Centralise, réutilise tes versions, et envoie en confiance." },
  },
  pricing: {
    title: "Tarifs",
    ...
    popular: "Populaire",
    choose: "Choisir",
    p1: { title: "Starter", price: "0€", desc: "Pour tester et démarrer.", f: ["Génération basique", "1 CV", "Support standard"] },
    p2: { title: "Pro", price: "9,99€", desc: "Le meilleur équilibre.", f: ["CV illimités", "Lettres illimitées", "Suivi candidatures", "Statistiques"] },
    p3: { title: "Elite", price: "19,99€", desc: "Pour les power users.", f: ["Tout Pro", "Modèles premium", "Mode vocal avancé", "Priorité support"] },
  },
  faq: {
    title: "FAQ",
    q: [
      { q: "Est-ce que c’est adapté à mon métier ?", a: "Oui. Tu peux préciser ton secteur, ton niveau et ton style. L’IA s’adapte." },
      { q: "Puis-je modifier après génération ?", a: "Bien sûr. Tu peux ajuster texte, structure et versions." },
      { q: "Mes données sont-elles sécurisées ?", a: "Nous appliquons des règles de sécurité et des contrôles d’accès côté Firebase." },
    ],
  },
  cta: { title: "Prêt à accélérer tes candidatures ?", subtitle: "Crée ton compte et commence en quelques secondes.", button: "Commencer" },
  footer: { brand: "Assistant IA", rights: "Tous droits réservés" },
};
    ...
    >
      ...
      <nav className="fixed top-0 inset-x-0 z-50 h-16 border-b border-[var(--border)] bg-[var(--bg)]/80 backdrop-blur-md transition-colors duration-300">
        <div className="max-w-7xl mx-auto px-4 h-full flex items-center justify-between">
          ...
          >
            ...
            <span className="font-bold tracking-tight text-[var(--ink)]">{COPY.brand}</span>
          </div>

          <div className="hidden md:flex items-center gap-8 text-sm font-medium text-[var(--muted)]">
            <button onClick={() => scrollTo("features")} className="hover:text-[var(--ink)] transition-colors">
              {COPY.nav.features}
            </button>
            <button onClick={() => scrollTo("process")} className="hover:text-[var(--ink)] transition-colors">
              {COPY.nav.process}
            </button>
            <button onClick={() => scrollTo("pricing")} className="hover:text-[var(--ink)] transition-colors">
              {COPY.nav.pricing}
            </button>
          </div>
      ...
      <main className="relative pt-32 pb-16">
        ...
        <section className="relative px-4 max-w-7xl mx-auto mb-32">
          ...
          <motion.div style={{ y: heroY, opacity: heroOpacity }} className="relative z-10 text-center flex flex-col items-center">
            <motion.div
              initial={{ opacity: 0, y: 20 }}
            ...
            >
              <div className="absolute inset-0 bg-gradient-to-t from-[var(--bg)] via-transparent to-transparent z-20 pointer-events-none" />
              ...
              <div className="grid grid-cols-[200px_1fr] h-[550px] opacity-90">
                ...
                <div className="border-r border-[var(--border)] bg-[var(--bg-soft)] p-4 flex flex-col gap-6">
                  <div className="flex items-center gap-3 px-2">
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="h-1.5 w-10 bg-black/10 dark:bg-white/10 rounded" />
                    </div>
                  </div>
                  ...
                  <div className="space-y-1">
                    <div className="flex items-center gap-3 px-3 py-2 rounded-lg bg-blue-500/10 border border-blue-500/20 text-blue-500 text-xs font-medium">
                      <LayoutDashboard className="w-3.5 h-3.5" /> {COPY.mock.dashboard}
                    </div>

                    {[
                      { icon: FileText, label: COPY.mock.cvs },
                      { icon: Briefcase, label: COPY.mock.apps },
                      { icon: PenTool, label: COPY.mock.letters },
                      { icon: BarChart3, label: COPY.mock.stats },
                    ].map((item, i) => (
                      <div
                        key={i}
                      ...
                      >
                        ...
                        <div className="h-2 w-24 bg-black/10 dark:bg-white/10 rounded" />
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="mt-auto p-3 rounded-xl bg-gradient-to-br from-indigo-900/20 to-purple-900/20 border border-[var(--border)]">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="text-[10px] font-bold text-indigo-300">{COPY.mock.proMode}</span>
                    </div>
                ...
                <div className="bg-transparent p-6 flex flex-col gap-6 relative">
                  <div className="flex items-center justify-between">
                    <div>
                      <h3 className="font-semibold text-lg text-[var(--ink)]">{COPY.mock.hello}</h3>
                      <p className="text-[var(--muted)] text-xs">{COPY.mock.subtitle}</p>
                    </div>
                    <div className="flex gap-3">
                      ...
                      <div className="w-8 h-8 rounded-full bg-black/5 dark:bg-white/5 flex items-center justify-center border border-[var(--border)] relative">
                        ...
                        <div className="absolute top-2 right-2 w-1.5 h-1.5 bg-red-500 rounded-full border border-[var(--bg-soft)]" />
                      </div>
                    </div>
                  </div>
                    ...
                    <div className="relative z-10 flex justify-between items-start">
                      <div className="space-y-3">
                        <div className="inline-flex items-center gap-2 px-2 py-1 rounded bg-blue-500/20 text-blue-400 text-[10px] font-bold uppercase tracking-wider border border-blue-500/20">
                          <Zap className="w-3 h-3" /> {COPY.mock.assistantActive}
                        </div>
                        <h4 className="text-xl font-bold text-[var(--ink)]">{COPY.mock.readyToApply}</h4>
                        <div className="h-8 w-64 bg-[var(--bg)]/40 rounded-lg border border-[var(--border)] flex items-center px-3 text-xs text-[var(--muted)]">
                          {COPY.mock.placeholder}
                        </div>
                      </div>
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    <div className="p-4 rounded-xl bg-[var(--bg-soft)] border border-[var(--border)] space-y-3">
                      ...
                      <div>
                        <div className="h-2 w-16 bg-black/20 dark:bg-white/20 rounded mb-1.5" />
                      ...
                      <div className="pt-2 border-t border-[var(--border)] flex items-center gap-2">
                        ...
                        <span className="text-[10px] text-emerald-500 font-bold">{COPY.mock.match}</span>
                      </div>
                    </div>
                    ...
                    <div className="p-4 rounded-xl bg-[var(--bg-soft)] border border-[var(--border)] space-y-3">
                      ...
                      <div>
                        <div className="h-2 w-20 bg-black/20 dark:bg-white/20 rounded mb-1.5" />
                        <div className="h-1.5 w-12 bg-black/10 dark:bg-white/10 rounded" />
                      </div>
                      <div className="pt-2 border-t border-[var(--border)] flex gap-1">
                        {[1, 2, 3].map((i) => (
                          <div key={i} className="h-1 w-1 rounded-full bg-[var(--muted)]" />
                        ))}
                      </div>
                    </div>
                    ...
                    <div className="p-4 rounded-xl bg-[var(--bg-soft)] border border-[var(--border)] space-y-3">
                      ...
                      <div className="flex items-end gap-0.5 h-8 opacity-50">
                        {[4, 8, 6, 3, 9, 5, 7, 4, 6, 3].map((h, i) => (
                          <div key={i} style={{ height: `${h * 10}%` }} className="w-1 bg-black/30 dark:bg-white/40 rounded-full" />
                        ))}
                      </div>
                      <div className="text-[10px] text-[var(--muted)] text-center">{COPY.mock.generatingPitch}</div>
                    </div>
                  </div>
                  ...
                  <div className="flex-1 rounded-xl bg-[var(--bg-soft)] border border-[var(--border)] p-4 space-y-3">
                    ...
                    {[1, 2, 3].map((i) => (
                      <div key={i} className="flex items-center gap-3 p-2 rounded hover:bg-black/5 dark:hover:bg-white/5 transition-colors">
                        <div className="w-8 h-8 rounded bg-blue-500/10 flex items-center justify-center text-blue-500 font-bold text-xs">
                          G
                        </div>
                        <div className="space-y-1 flex-1">
                          <div className="h-1.5 w-20 bg-black/20 dark:bg-white/20 rounded" />
        ...
        <section id="features" className="max-w-7xl mx-auto px-4 py-24">
          <div className="text-center mb-16">
            <h2 className="text-3xl md:text-4xl font-bold mb-4 text-[var(--ink)]">{COPY.features.title}</h2>
            <p className="text-[var(--muted)] max-w-2xl mx-auto">{COPY.features.subtitle}</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            >
              ...
              <div className="relative z-10">
                ...
                <h3 className="text-xl font-bold mb-2 text-[var(--ink)]">{COPY.features.f1.title}</h3>
                <p className="text-[var(--muted)] max-w-sm">{COPY.features.f1.desc}</p>
              </div>
            </motion.div>
            ...
            >
              ...
              <h3 className="text-xl font-bold mb-2 text-[var(--ink)]">{COPY.features.f2.title}</h3>
              <p className="text-[var(--muted)] text-sm">{COPY.features.f2.desc}</p>
            </motion.div>
            ...
            >
              ...
              <h3 className="text-xl font-bold mb-2 text-[var(--ink)]">{COPY.features.f3.title}</h3>
              <p className="text-[var(--muted)] text-sm">{COPY.features.f3.desc}</p>
            </motion.div>
            ...
            >
              ...
              <div className="relative z-10">
                ...
                <h3 className="text-xl font-bold mb-2 text-[var(--ink)]">{COPY.features.f4.title}</h3>
                <p className="text-[var(--muted)] max-w-sm">{COPY.features.f4.desc}</p>
              </div>
            </motion.div>
          </div>
        </section>
        ...
        <section id="process" className="max-w-5xl mx-auto px-4 py-24">
          <h2 className="text-3xl md:text-4xl font-bold mb-16 text-center text-[var(--ink)]">{COPY.process.title}</h2>

          <div className="relative border-l border-[var(--border)] ml-6 md:ml-12 space-y-16">
            {[
              { step: "01", ...COPY.process.s1 },
              { step: "02", ...COPY.process.s2 },
              { step: "03", ...COPY.process.s3 },
            ].map((item, index) => (
              <motion.div
                key={index}
              ...
              >
                ...
                <p className="text-[var(--muted)] text-lg leading-relaxed max-w-xl">{item.desc}</p>
              </motion.div>
            ))}
          </div>
        </section>
        ...
        >
          <div className="text-center mb-16">
            <h2 className="text-3xl md:text-4xl font-bold mb-4 text-[var(--ink)]">{COPY.pricing.title}</h2>
            <p className="text-[var(--muted)]">{COPY.pricing.subtitle}</p>
          </div>

          <div className="grid md:grid-cols-3 gap-8 max-w-5xl mx-auto">
            {[
              { title: COPY.pricing.p1.title, price: COPY.pricing.p1.price, desc: COPY.pricing.p1.desc, features: COPY.pricing.p1.f },
              { title: COPY.pricing.p2.title, price: COPY.pricing.p2.price, desc: COPY.pricing.p2.desc, features: COPY.pricing.p2.f, popular: true },
              { title: COPY.pricing.p3.title, price: COPY.pricing.p3.price, desc: COPY.pricing.p3.desc, features: COPY.pricing.p3.f },
            ].map((plan, i) => (
              <motion.div
                key={i}
                whileHover={{ y: -10 }}
        ...
        <section id="faq" className="max-w-3xl mx-auto px-4 py-24">
          <h2 className="text-3xl font-bold mb-12 text-center text-[var(--ink)]">{COPY.faq.title}</h2>
          <div className="space-y-4">
            {COPY.faq.q.map((item, i) => (
              <details
                key={i}
              ...
              >
                ...
                <div className="px-6 pb-6 text-[var(--muted)] text-sm leading-relaxed">{item.a}</div>
              </details>
            ))}
          </div>
        </section>
        ...
        <section className="px-4 py-24 text-center">
          <div className="max-w-4xl mx-auto bg-gradient-to-b from-blue-900/20 to-[var(--bg-soft)] border border-blue-500/20 rounded-[3rem] p-12 relative overflow-hidden">
            ...
            <h2 className="text-3xl md:text-5xl font-bold mb-6 relative z-10 text-[var(--ink)]">{COPY.cta.title}</h2>
            <p className="text-lg text-[var(--muted)] mb-10 max-w-2xl mx-auto relative z-10">{COPY.cta.subtitle}</p>
      ...
      <footer className="border-t border-[var(--border)] bg-black/5 dark:bg-black/20 text-[var(--muted)] py-12 px-4 text-center text-sm">
        <div className="flex items-center justify-center gap-2 mb-4">
          ...
          <span className="font-bold text-[var(--ink)]">{COPY.footer.brand}</span>
        </div>
